...
public class CarSelection : MonoBehaviour
{
    ...
    // {
    //     for (int i = 0; i < saveManager.carsUnlocked.Length; i++)
    //     {
    //         if (saveManager.carsUnlocked[i] && i < prefabs.Length) // проверяем, разблокирован ли префаб и находится ли его индекс в пределах массива prefabs
    //         {
    //             Instantiate(prefabs[i], carSelection.transform.position, carSelection.transform.rotation, carSelection.transform);
    ...
    private void SelectCar(int _index)
    {
        ...
        {
            //skinObjects[i].SetActive(SaveManager.instance.carsUnlocked[i]);
        ...
        UpdateUI(); // Обновляем состояние кнопок Play/Buy
    }
